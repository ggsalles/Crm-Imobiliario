        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {loading ? (
              ...
              <div key={company.id} className="bg-white p-6 rounded-2xl border shadow-sm hover:shadow-md transition-shadow group flex flex-col justify-between">
                <div>
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center font-bold text-xl text-slate-600">
                  ...
                  {company.website && (
                    <div className="flex items-center gap-2 text-sm text-blue-600 hover:underline mb-4">
                ...
                <button className="w-full text-sm font-bold py-2 bg-slate-50 text-slate-600 rounded-xl hover:bg-slate-100 transition-all">Ver Contatos</button>
              </div>
            ))}
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="bg-white rounded-3xl p-8 w-full max-w-lg relative shadow-2xl overflow-hidden">
              ...
              <h2 className="text-2xl font-bold mb-6">{editingCompany ? 'Editar Empresa' : 'Nova Empresa'}</h2>
              <form onSubmit={handleSave} className="space-y-4">
                <div>
                  <label className="text-xs font-bold uppercase text-muted-foreground mb-1 block">Nome da Empresa</label>
                  <input name="name" required defaultValue={editingCompany?.name} placeholder="Ex: Tech Solutions Ltda" className="w-full px-4 py-3 rounded-xl border bg-muted/20 focus:outline-none focus:ring-2 focus:ring-primary/20" />
                </div>
                <div>
                  <label className="text-xs font-bold uppercase text-muted-foreground mb-1 block">Setor / Atividade</label>
                  <input name="industry" defaultValue={editingCompany?.industry} placeholder="Ex: Tecnologia da Informação" className="w-full px-4 py-3 rounded-xl border bg-muted/20 focus:outline-none focus:ring-2 focus:ring-primary/20" />
                </div>
                <div>
                  <label className="text-xs font-bold uppercase text-muted-foreground mb-1 block">Website (opcional)</label>
